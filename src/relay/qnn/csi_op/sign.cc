...
bool QnnCSISignRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
...
Expr MakeQnnCSISign(Expr data, double input_scale, int32_t input_zero_point, double output_scale,
  ...
  static const Op& op = Op::Get("qnn.csi.sign");
  return Call(op, {data}, Attrs(attrs), {});
}

RELAY_REGISTER_OP("qnn.csi.sign")
    .describe(R"code(Returns the sign input array, computed element-wise.
...
.. math::
   sign(x)
    ...
    .add_type_rel("QnnCSISignRel", QnnCSISignRel)
    .set_attr<TOpPattern>("TOpPattern", kOpaque);

TVM_REGISTER_GLOBAL("relay.qnn.op._make.CSISign").set_body_typed(MakeQnnCSISign);
