...
// relay.op.qnn.conv2d
TVM_REGISTER_NODE_TYPE(QnnCSIConv2DChannelAttrs);

bool QnnCSIConv2DChannelRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  const auto* param = attrs.as<QnnCSIConv2DChannelAttrs>();
  CHECK(param != nullptr) << "QnnCSIConv2DChannelAttrs cannot be nullptr.";
...
Expr MakeQnnCSIConv2DChannel(Expr data, Expr weight, Expr bias, Expr kernel_scale,
                             Expr kernel_zero_point, double input_scale, int32_t input_zero_point,
                             double output_scale, int32_t output_zero_point,
                             ...
                             String layer_name) {
  auto attrs = make_object<QnnCSIConv2DChannelAttrs>();
  ...
  static const Op& op = Op::Get("qnn.csi.conv2d_channel");
  return Call(op, {data, weight, bias, kernel_scale, kernel_zero_point}, Attrs(attrs), {});
}

RELAY_REGISTER_OP("qnn.csi.conv2d_channel")
...
- **out**:  This depends on the `layout` parameter. Output is 4D array of shape
    ...
    .set_attrs_type<QnnCSIConv2DChannelAttrs>()
    .set_num_inputs(5)
    ...
    .add_argument("kernel_scale", "Tensor", "The quantized kernel scale tensor.")
    .add_argument("kernel_zero_point", "Tensor", "The quantized kernel zero point tensor.")
    .set_support_level(11)
    .add_type_rel("QnnCSIConv2DChannelRel", QnnCSIConv2DChannelRel)
    .set_attr<TOpPattern>("TOpPattern", kOpaque);

TVM_REGISTER_GLOBAL("relay.qnn.op._make.CSIConv2DChannel").set_body_typed(MakeQnnCSIConv2DChannel);

Expr MakeQnnCSIConv2DReluChannel(
    Expr data, Expr weight, Expr bias, Expr kernel_scale, Expr kernel_zero_point,
    double input_scale, int32_t input_zero_point, double output_scale, int32_t output_zero_point,
    Array<IndexExpr> strides, Array<IndexExpr> padding, Array<IndexExpr> dilation, int groups,
  ...
  attrs->max_values = std::move(max_values);
  attrs->min_values = std::move(min_values);
  static const Op& op = Op::Get("qnn.csi.conv2d_relu_channel");
  return Call(op, {data, weight, bias, kernel_scale, kernel_zero_point}, Attrs(attrs), {});
}

RELAY_REGISTER_OP("qnn.csi.conv2d_relu_channel")
    .describe(R"code(2D quantized convolution layer.
This operator convolves quantized weight with quantized data. The scale of the
output quantized tensor is the product of the weight_scale and input_scale of
the input quantized tensors. The zero point of the output quantized tensor is
...
TVM_REGISTER_GLOBAL("relay.qnn.op._make.CSIConv2DReluChannel")
    .set_body_typed(MakeQnnCSIConv2DReluChannel);

Expr MakeQnnCSIConv2DRelu6Channel(
    Expr data, Expr weight, Expr bias, Expr kernel_scale, Expr kernel_zero_point,
    double input_scale, int32_t input_zero_point, double output_scale, int32_t output_zero_point,
    Array<IndexExpr> strides, Array<IndexExpr> padding, Array<IndexExpr> dilation, int groups,
    IndexExpr channels, Array<IndexExpr> kernel_size, std::string data_layout,
  ...
  attrs->max_values = std::move(max_values);
  attrs->min_values = std::move(min_values);
  static const Op& op = Op::Get("qnn.csi.conv2d_relu6_channel");
  return Call(op, {data, weight, bias, kernel_scale, kernel_zero_point}, Attrs(attrs), {});
}

RELAY_REGISTER_OP("qnn.csi.conv2d_relu6_channel")
    .describe(R"code(2D quantized convolution layer.
This operator convolves quantized weight with quantized data. The scale of the
output quantized tensor is the product of the weight_scale and input_scale of
the input quantized tensors. The zero point of the output quantized tensor is
...
TVM_REGISTER_GLOBAL("relay.qnn.op._make.CSIConv2DRelu6Channel")
    .set_body_typed(MakeQnnCSIConv2DRelu6Channel);
