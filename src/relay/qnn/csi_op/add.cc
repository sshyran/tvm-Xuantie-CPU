...
// QNN Addition operator.
Expr MakeQnnCSIBiasAdd(Expr lhs, Expr rhs, double lhs_scale, int32_t lhs_zero_point,
...
RELAY_REGISTER_OP("qnn.csi.bias_add")
    ...
    .add_argument("rhs", "Tensor", "The right hand side quantized tensor.")
    .set_support_level(11)
    .add_type_rel("QnnCSIBiasAddcast", QnnCSIBinaryOpRel)
    .set_attr<TOpPattern>("TOpPattern", kOpaque);

TVM_REGISTER_GLOBAL("relay.qnn.op._make.CSIBiasAdd").set_body_typed(MakeQnnCSIBiasAdd);

// QNN Subtract operator.
Expr MakeQnnCSISubtract(Expr lhs, Expr rhs, double lhs_scale, int32_t lhs_zero_point,
                        double rhs_scale, int32_t rhs_zero_point, double output_scale,
                        int32_t output_zero_point, Array<IndexExpr> max_values,
                        Array<IndexExpr> min_values, String layer_name) {
  auto attrs = make_object<QnnBinaryOpAttrs>();
