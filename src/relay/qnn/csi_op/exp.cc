...
bool QnnCSIExpRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
...
Expr MakeQnnCSIExp(Expr data, double input_scale, int32_t input_zero_point, double output_scale,
  ...
  static const Op& op = Op::Get("qnn.csi.exp");
  return Call(op, {data}, Attrs(attrs), {});
}

RELAY_REGISTER_OP("qnn.csi.exp")
...
.. math::
   exp(x)
    ...
    .add_type_rel("QnnCSIExpRel", QnnCSIExpRel)
    .set_attr<TOpPattern>("TOpPattern", kOpaque);

TVM_REGISTER_GLOBAL("relay.qnn.op._make.CSIExp").set_body_typed(MakeQnnCSIExp);
