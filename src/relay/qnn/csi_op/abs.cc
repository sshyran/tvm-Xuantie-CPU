...
bool QnnCSIAbsRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
...
Expr MakeQnnCSIAbs(Expr data, double input_scale, int32_t input_zero_point, double output_scale,
  ...
  static const Op& op = Op::Get("qnn.csi.abs");
  return Call(op, {data}, Attrs(attrs), {});
}

RELAY_REGISTER_OP("qnn.csi.abs")
    .describe(R"code(Returns the abs input array, computed element-wise.
...
.. math::
   abs(x)
    ...
    .add_type_rel("QnnCSIAbsRel", QnnCSIAbsRel)
    .set_attr<TOpPattern>("TOpPattern", kOpaque);

TVM_REGISTER_GLOBAL("relay.qnn.op._make.CSIAbs").set_body_typed(MakeQnnCSIAbs);
