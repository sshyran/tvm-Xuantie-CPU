...
bool QnnCSICastRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
...
Expr MakeQnnCSICast(Expr data, double input_scale, int32_t input_zero_point, double output_scale,
  ...
  static const Op& op = Op::Get("qnn.csi.cast");
  return Call(op, {data}, Attrs(attrs), {});
}

RELAY_REGISTER_OP("qnn.csi.cast")
    ...
    .add_type_rel("QnnCSICastRel", QnnCSICastRel)
    .set_attr<TOpPattern>("TOpPattern", kOpaque);

TVM_REGISTER_GLOBAL("relay.qnn.op._make.CSICast").set_body_typed(MakeQnnCSICast);
