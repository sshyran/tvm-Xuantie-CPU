...
Expr MakeQnnCSISplit(Expr data, ObjectRef indices_or_sections, int axis, double input_scale,
                     int32_t input_zero_point, Array<tvm::PrimExpr> output_scales,
                     Array<Integer> output_zero_points, DataType out_dtype,
  ...
  attrs->output_scales = output_scales;
  attrs->output_zero_points = std::move(output_zero_points);
...
TVM_REGISTER_GLOBAL("relay.qnn.op._make.CSISplit")
    .set_body([](const TVMArgs& args, TVMRetValue* rv) {
      if (args.type_codes[1] == kDLInt) {
        ...
        *rv = MakeQnnCSISplit(
            args[0], tir::make_const(DataType::Int(32), static_cast<int>(args[1])), args[2],
            args[3], args[4], args[5], args[6], args[7], args[8], args[9], args[10]);
      } else {
        *rv = MakeQnnCSISplit(args[0], args[1], args[2], args[3], args[4], args[5], args[6],
