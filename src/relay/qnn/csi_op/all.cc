...
bool QnnCSIAllRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
...
Expr MakeQnnCSIAll(Expr data, double input_scale, int32_t input_zero_point, double output_scale,
  ...
  static const Op& op = Op::Get("qnn.csi.all");
  return Call(op, {data}, Attrs(attrs), {});
}

RELAY_REGISTER_OP("qnn.csi.all")
    .describe(R"code(Returns the all input array, computed element-wise.
...
.. math::
   all(x)
    ...
    .add_type_rel("QnnCSIAllRel", QnnCSIAllRel)
    .set_attr<TOpPattern>("TOpPattern", kOpaque);

TVM_REGISTER_GLOBAL("relay.qnn.op._make.CSIAll").set_body_typed(MakeQnnCSIAll);
