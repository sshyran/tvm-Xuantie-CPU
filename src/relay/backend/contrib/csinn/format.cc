...
std::vector<string> CSINNTensor::serialize(size_t qoffset, size_t coffset) {
  ...
  if (tensor->is_const) {
    ...
    if (tensor->mtype == CSINN_MEM_TYPE_ASP42 || tensor->mtype == CSINN_MEM_TYPE_ASP41) {
...
void CSINNVarTensor::to_file(std::ofstream& file) {
  file.write(reinterpret_cast<char*>(quant_data), binary_model_quant_size);
}

void CSINNConstantTensor::to_file(std::ofstream& file) {
  file.write(reinterpret_cast<char*>(quant_data), binary_model_quant_size);
  file.write(reinterpret_cast<char*>(const_data), binary_model_const_size);
...
std::vector<string> CSINNOP::serialize() {
  ...
  for (auto tensor : inputs) {
    coffset += tensor->binary_model_quant_size;
    std::vector<string> str = tensor->serialize(qoffset, coffset);
    tensor->const_offset = coffset;
    tensor->qinfo_offset = qoffset;
    for (auto s : str) {
      strs.push_back(s);
    }
