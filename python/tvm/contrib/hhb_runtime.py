...
def create(libmod, origin_mod, ctx, output_dir="."):
    ...
    ctx : Device or list of Device
        The context to deploy the module. It can be local or remote when there
        is only one Device. Otherwise, the first context in the list will
...
def get_device_ctx(libmod, ctx):
    ...
    if isinstance(ctx, Device):
        ctx = [ctx]
    elif not isinstance(ctx, (list, tuple)):
        raise ValueError("ctx has to be the type of Device or a list of " "TVMCDevice")
    for cur_ctx in ctx:
        if not isinstance(cur_ctx, Device):
            raise ValueError("ctx has to be the type of Device or a list " "of Device")
