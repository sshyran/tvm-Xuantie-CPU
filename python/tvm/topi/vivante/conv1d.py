...
"""Compute definition for conv1d with vivante backend"""
import tvm
from tvm import te
...
@autotvm.register_topi_schedule("conv1d_ncw.vivante")
def schedule_conv1d_ncw(cfg, outs):
    """TOPI schedule callback of conv1d ncw for vivante gpu
    ...
    def _callback(op):
        if op.tag == "conv1d_ncw":
            ...
            cfg.fallback_split("tile_n", [-1, 1, 1, 2])
            cfg.fallback_split("tile_f", [-1, 1, 1, 2])
            cfg.fallback_split("tile_x", [-1, 1, 1, 2])
...
@autotvm.register_topi_schedule("conv1d_nwc.vivante")
def schedule_conv1d_nwc(cfg, outs):
    """TOPI schedule callback of conv1d nwc for vivante gpu
    ...
    def _callback(op):
        if op.tag == "conv1d_nwc":
            ...
            # To get correct result in vivante target
            cfg.fallback_split("tile_n", [-1, 1, 1, 2])
            cfg.fallback_split("tile_f", [-1, 1, 1, 2])
            cfg.fallback_split("tile_x", [-1, 1, 1, 2])
